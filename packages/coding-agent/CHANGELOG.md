...
- Interactive Codex sessions backed by local credential storage now re-import the current Codex CLI OAuth credential after a definitive refresh rejection, including pinned `--credential` lookups. Recovery uses Codex-only discovery, preserves the trigger across bounded transient state/source read failures, cancels on logout or another credential-generation change, and still requires prior import consent; broker-backed sessions remain explicitly excluded until broker failures transport a definitive provider classification.
